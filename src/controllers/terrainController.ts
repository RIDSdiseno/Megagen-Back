...
import { RolUsuario, TerrenoTipo, VisitaEstado } from "@prisma/client";
...
export async function listTerrainHistory(req: Request, res: Response) {
  try {
    ...
    const where: any = {};
    if (bodegueroEmail) where.bodeguero = { email: bodegueroEmail };
    if (tipo && Object.values(TerrenoTipo).includes(tipo as TerrenoTipo)) where.tipo = tipo as TerrenoTipo;
    if (from || to) where.fecha = { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) };
    if (cliente) where.cotizacion = { cliente: { contains: cliente, mode: "insensitive" } };
    if (user?.rol === RolUsuario.TRABAJADOR) where.bodegueroId = user.id;

    const registros = await prisma.terrenoHistorial.findMany({
      where,
      orderBy: { fecha: "desc" },
...
export async function createTerrainHistory(req: Request, res: Response) {
  const { user } = req as AuthRequest;
  if (!user) return res.status(401).json({ message: "No autenticado" });

  const { fecha, tipo, titulo, detalle, ubicacion, documento, bodegueroEmail, cotizacionId } = req.body ?? {};
  if (!fecha || !titulo) return res.status(400).json({ message: "fecha y titulo son obligatorios" });

  const allowedRoles: RolUsuario[] = [RolUsuario.ADMINISTRADOR, RolUsuario.SUPERVISOR, RolUsuario.TRABAJADOR];
  if (!allowedRoles.includes(user.rol)) {
    return res.status(403).json({ message: "No autorizado" });
  }

  try {
    let bodegueroId: number | null = null;
    if (bodegueroEmail) {
      const bode = await prisma.user.findUnique({ where: { email: bodegueroEmail } });
      bodegueroId = bode?.id ?? null;
    }

    const created = await prisma.terrenoHistorial.create({
      data: {
        fecha: new Date(fecha),
        tipo: Object.values(TerrenoTipo).includes(tipo) ? (tipo as TerrenoTipo) : TerrenoTipo.SALIDA,
        titulo,
        detalle: detalle || "",
        ubicacion: ubicacion || "",
        documento: documento || "",
...
export async function listTerrainVisits(req: Request, res: Response) {
  try {
    const { user } = req as AuthRequest;
    const bodegueroEmail = typeof req.query.bodegueroEmail === "string" ? req.query.bodegueroEmail : "";
    ...
    const where: any = {};
    if (bodegueroEmail) where.bodeguero = { email: bodegueroEmail };
    if (estado && Object.values(VisitaEstado).includes(estado as VisitaEstado)) where.estado = estado as VisitaEstado;
    if (from || to) where.fecha = { ...(from ? { gte: from } : {}), ...(to ? { lte: to } : {}) };
    if (cliente) where.cliente = { contains: cliente, mode: "insensitive" };
    if (user?.rol === RolUsuario.TRABAJADOR) where.bodegueroId = user.id;

    const visitas = await prisma.visitaTerreno.findMany({
      where,
      orderBy: { fecha: "desc" },
      include: { bodeguero: true, cotizacion: true },
    });
...
export async function createTerrainVisit(req: Request, res: Response) {
  const { user } = req as AuthRequest;
  if (!user) return res.status(401).json({ message: "No autenticado" });
  ...
  try {
    ...
    if (bodegueroEmailFinal) {
      const bode = await prisma.user.findUnique({ where: { email: bodegueroEmailFinal } });
      bodegueroId = bode?.id ?? null;
      bodegueroEmailFinal = bode?.email || bodegueroEmailFinal;
    }

    let cotizacionData:
      | { id: number; codigo: string; cliente: string | null; direccion: string | null }
      | null = null;
    const cotIdNum = cotizacionId ? Number(cotizacionId) : null;
    if (cotIdNum) {
      cotizacionData = await prisma.cotizacion.findUnique({
        where: { id: cotIdNum },
    ...
    const estadoValido = Object.values(VisitaEstado).includes(estado as VisitaEstado)
      ? (estado as VisitaEstado)
      : VisitaEstado.PROGRAMADA;

    const created = await prisma.visitaTerreno.create({
      data: {
        fecha: new Date(fecha),
        estado: estadoValido,
